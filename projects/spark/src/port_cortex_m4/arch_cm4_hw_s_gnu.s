/**
 * @file arch_xx_hw_s_gnu.c
 * @author 文佳源 (648137125@qq.com)
 * @brief catOS可移植硬件部分-汇编语言部分-gnu格式
 * @version 0.1
 * @date 2025-03-10
 * 
 * Copyright (c) 2025
 ...
 * primask-->r0
 */
    .global _cat_hw_irq_disable
    .type _cat_hw_irq_disable, %function
_cat_hw_irq_disable:
 ...
 * r0-->status
 */
    .global _cat_hw_irq_enable
    .type _cat_hw_irq_enable, %function
_cat_hw_irq_enable:
...
cat_hw_context_switch:
    ...
    /* cat_context_from_task_sp_ptr = &(cat_task_current->sp) */
...
PendSV_Handler:
    ...
    /* 保存浮点单元 */
    tst lr, #0x10 /* EXC_RETURN[4]保存的是浮点单元使能标志(CONTROL.FPCA取反) */
    it  eq        /* 注意eq为判断Z==1即结果为0, 如果EXC_RETURN[4]==0则保存浮点寄存器 */
    vstmdbeq r1!, {s16-s31}

    /* 保存 r4-r11（减前/递减满堆栈） */
    /* cm4额外保存EXC_RETURN(此处不是真正的lr, 原本的lr已经被压进栈了) */
    stmfd r1!, {r4-r11, lr}
