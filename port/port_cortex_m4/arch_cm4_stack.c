...
struct _init_stack_frame
{
    struct _stack_frame manual_save_frame;
    cat_u32 init_exc_return;
    struct _exception_stack_frame exeption_stack_frame;
};

struct _running_stack_frame
{
    struct _stack_frame manual_save_frame;
    struct _exception_stack_frame exeption_stack_frame;
};
...
{
  struct _init_stack_frame *stack_frame;
  cat_u8             *stack;
  cat_u32            i;
  ...
  /* 栈向上生长? */
  stack -= sizeof(struct _init_stack_frame);
  stack_frame = (struct _init_stack_frame *)stack;

  /* 初始化栈帧中所有寄存器 */
  for(i = 0; i < (sizeof(struct _init_stack_frame) / sizeof(cat_u32)); i++)
  {
  ...
  stack_frame->init_exc_return = 0xfffffffd;
 ...
 */
void catos_hard_fault_deal(struct _running_stack_frame *stack)
{
    cat_kprintf("\r\n\r\n*** HardFault occurred ***\r\n");
    cat_u32 exc_ret = *((cat_u32*)stack); /* 这里其实是最后压栈的lr */
    cat_kprintf("exrt: 0x%8x\r\n", exc_ret);
    stack = (struct _running_stack_frame *)((cat_ubase)stack+4);
