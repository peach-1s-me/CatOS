...
struct _cat_shell_instance_t
{
    struct
    {
        ...
        cat_u32 current_combine_key_val;     /**< 当前累计的键值*/
        cat_u8  current_combine_key_offset;  /**< 当前组合键偏移*/
    } buffer;
    struct
    {
        /* 历史记录：用上下键循环显示 */
...
#define CAT_SHELL_CLR_SCREEN        "\033[2J\033[0;0H"
    ...
    #define IMPORT_SECTION(x) \
        extern cat_u32 x##$$Base; \
        extern cat_u32 x##$$Limit;

    #define SECTION_START(x)  (&x##$$Base)
    #define SECTION_END(x)    (&x##$$Limit)
    #define SECTION_SIZE(x)   ((cat_u32)&x##$$Limit - (cat_u32)&x##$$Base)
